"""
HyperMARL RNN for Alem with shared recurrent backbone.
"""
...
class ActorCriticRNN(nn.Module):
    ...
    def setup(self):
        ...
        self.hyper_hidden_dims = self.config.get("HYPERNET_HIDDEN_DIMS", [64])
        ...
        self.activation_fn = get_activation(self.config.get("ACTIVATION", "relu"))
        ...
        if self.use_agent_id_embeddings:
            ...
            self.agent_embeddings = jnp.eye(self.num_agents, dtype=jnp.float32)

        # Flax GRUCell uses h.shape[-1] internally, so RNN output width = carry width = GRU_HIDDEN_DIM
        # (the `features` kwarg is ignored). Actor head mirrors ippo_rnn.py (GRU_HIDDEN_DIM wide);
        # critic head uses FC_DIM_SIZE to match the narrower critic head in ippo_rnn.py.
        rnn_output_dim = self.config["GRU_HIDDEN_DIM"]
        actor_hidden_dim = self.config["GRU_HIDDEN_DIM"]
        critic_hidden_dim = self.config["FC_DIM_SIZE"]

        self.actor_output_dims = [
            (rnn_output_dim, actor_hidden_dim),
            (actor_hidden_dim, self.action_dim),
        ]
        self.critic_output_dims = [
            (rnn_output_dim, critic_hidden_dim),
            (critic_hidden_dim, 1),
        ]
        ...
        self.actor_hypernet = MLPHyperNetwork(
            ...
            hidden_dims=self.hyper_hidden_dims,
    ...
    def __call__(self, hidden, x):
        ...
        agent_one_hot = obs_with_id[..., -self.num_agents :]
        agent_id = jnp.argmax(agent_one_hot, axis=-1).astype(jnp.int32)
        obs = obs_with_id[..., : -self.num_agents]
        ...
        def apply_hypernet(x_in, weights, biases, layer_defs):
            x_local = x_in
            for (in_dim, out_dim), w, b in zip(layer_defs[:-1], weights[:-1], biases[:-1]):
                w = w.reshape(in_dim, out_dim)
                x_local = self.activation_fn(jnp.matmul(x_local, w) + b)
            fin_in, fin_out = layer_defs[-1]
            return jnp.matmul(x_local, weights[-1].reshape(fin_in, fin_out)) + biases[-1]

        vmap_apply_hypernet = jax.vmap(apply_hypernet, in_axes=(None, 0, 0, None))
        all_actor_out = vmap_apply_hypernet(
            embedding, actor_weights, actor_biases, self.actor_output_dims
        )
        action_logits = all_actor_out[
            agent_id,
            jnp.arange(embedding.shape[0])[:, None],
            jnp.arange(embedding.shape[1]),
        ]

        pi = distrax.Categorical(logits=action_logits)

        all_critic_out = vmap_apply_hypernet(
            embedding, critic_weights, critic_biases, self.critic_output_dims
        )
        critic = all_critic_out[
            agent_id,
            jnp.arange(embedding.shape[0])[:, None],
            jnp.arange(all_critic_out.shape[2]),
        ]
        ...
        def _update_step(update_runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                if config.get("ACTION_MASKING", False):
                    inner_env = env._env._env if hasattr(env._env, "_env") else env._env
                    mask = jax.vmap(
...
def single_run(config):
    alg_name = config.get("ALG_NAME", "hypermarl-rnn")
    ...
    if config.get("APPEND_AGENT_ID", True):
    ...
    _obs_dim = env.observation_space(env.agents[0]).shape[0]
    ...
    network = ActorCriticRNN(
        ...
        config=config,
        num_agents=env.num_agents,
        observation_dim=env.observation_space(env.agents[0]).shape[0],
    )
    ...
    def ippo_policy_fn(hstate, obs_batch, done_batch, rng, avail_actions):
        """Policy fn for HyperMARL: shared network with per-agent ID conditioning."""
...
@hydra.main(
    version_base=None,
    config_path="config",
    config_name="hypermarl_rnn.yaml",
)
def main(config):
