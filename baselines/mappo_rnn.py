        ...
        def _update_step(update_runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                if config.get("ACTION_MASKING", False):
                    inner_env = (
                        env._env._env
                    )  # LogWrapper -> WorldStateWrapper -> AlemCoopSymbolicEnv
                    mask = jax.vmap(
                ...
                env_act = unbatchify_actions(action, env.agents, config["NUM_ENVS"], env.num_agents)
                # VALUE
            ...
            def callback(metric):
                ...
                if metric["returned_episode"].any():
                    to_log.update(
                        jax.tree.map(
                            lambda x: x[metric["returned_episode"]].mean(), metric["user_info"]
                        )
                    )
                    to_log["mean_episode_length"] = metric["returned_episode_lengths"][
                        metric["returned_episode"]
                    ].mean()
                    to_log["mean_episode_return"] = metric["returned_episode_returns"][
                        metric["returned_episode"]
                    ].mean()
                wandb.log(to_log, step=metric["update_steps"])
...
def single_run(config):
    ...
    _wrapped_env = WorldStateWrapper(env)
    _actor = ActorRNN(env.action_space(env.agents[0]).n, config=config)
    _critic = CriticRNN(config=config)
    _init_hstate = ScannedRNN.initialize_carry(1, config["GRU_HIDDEN_DIM"])
    _actor_params = _actor.init(
        jax.random.PRNGKey(0),
        _init_hstate,
        (jnp.zeros((1, 1, env.observation_space(env.agents[0]).shape[0])), jnp.zeros((1, 1))),
    )
    _critic_params = _critic.init(
        jax.random.PRNGKey(0),
        _init_hstate,
        (jnp.zeros((1, 1, _wrapped_env.world_state_size())), jnp.zeros((1, 1))),
    )
    num_params = sum(x.size for x in jax.tree.leaves(_actor_params)) + sum(
        x.size for x in jax.tree.leaves(_critic_params)
    )
    ...
    if load_ckpt:
        ...
        trained_actor_params = restored["actor"]["params"]
        trained_critic_params = restored["critic"]["params"]
    else:
        ...
        # Save checkpoint (actor + critic as dict)
        actor_train_state_vmapped = outs["runner_state"][0][0][0]
        critic_train_state_vmapped = outs["runner_state"][0][0][1]
        trained_actor_state = jax.tree.map(lambda x: x[0], actor_train_state_vmapped)
        trained_critic_state = jax.tree.map(lambda x: x[0], critic_train_state_vmapped)
        save_checkpoint({"actor": trained_actor_state, "critic": trained_critic_state}, config)
        trained_actor_params = trained_actor_state.params
        trained_critic_params = trained_critic_state.params

    actor_network = ActorRNN(env.action_space(env.agents[0]).n, config=config)
    critic_network = CriticRNN(config=config)
    # For MAPPO, we need both actor and critic
    trained_params = (trained_actor_params, trained_critic_params)
    network = (actor_network, critic_network)
    ...
    if config.get("VISUALIZE", True):
        ...
        run_visualization_mappo_rnn(config, env, trained_params, vis_rng, network, ScannedRNN)

    # Run final evaluation
    def mappo_policy_fn(hstate, obs_batch, done_batch, rng, avail_actions):
        """Policy fn for MAPPO: obs_batch (num_agents, obs_dim) -> actions (num_agents,)"""
        ac_in = (obs_batch[None, :], done_batch[None, :])
        hstate, pi = actor_network.apply(trained_actor_params, hstate, ac_in)
    ...
    run_final_eval(config, env, mappo_policy_fn, eval_hstate, eval_rng)
